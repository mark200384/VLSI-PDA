...
pair<int, int> VSlice(pair<int, int> a, pair<int, int> b) {
	pair<int, int > temp;
	temp.second = max(a.second, b.second);
	temp.first = a.first + b.first;
	return temp;
}
Dimension VMerge(Dimension first, Dimension second) {
	int i = 0, j = 0;
	Dimension dim_t;
	while (i < first.orients.size() && j < second.orients.size()) {
		dim_t.orients.push_back(VSlice(first.orients[i], second.orients[j]));
...
pair<int, int> HSlice(pair<int, int> a, pair<int, int>  b) {
	pair<int, int > temp;
	temp.first = max(a.first, b.first); //max of width
	temp.second = a.second + b.second;//sum of height
	return temp;
}
Dimension HMerge(Dimension first, Dimension second) {
	int i = 0, j = 0;
	Dimension dim_t;
	while (i < first.orients.size() && j < second.orients.size()) {
		dim_t.orients.push_back(HSlice(first.orients[i], second.orients[j]));
		dim_t.use.push_back(0);
		/*if (i == first.orients.size() - 1) j++;//i不能再往後
		else if (j == first.orients.size() - 1) i++;//j不能往後
...
void stockmeyer(vector<string> expression, vector<Block>& block_t) {
	...
	for (int i = 0; i < expression.size(); i++) {
		if (expression[i] != "H" && expression[i] != "V") {
			...
			if (expression[i] == "V") {
				res = VMerge(first, second);
			}
			else if (expression[i] == "H") {
				res = HMerge(first, second);
			}
...
int main(int argc, char* argv[]) {
  ...
  read_beg = clock();
	int total_block_area = read_hardblock(argv[1]);
	read_net(argv[2]);
	read_pl(argv[3]);
  read_end = clock();
  cout << "I/O time:" << (double)(read_end - read_beg)/ CLOCKS_PER_SEC <<endl;
  ...
  clock_t initfloorplan_beg, initfloorplan_end;
  initfloorplan_beg = clock();
	...
	initfloorplan_end = clock();
  int flag = 0;
  clock_t SA_beg, SA_end;
  SA_beg=clock();
  cout << "initfloorplan time:" << (double)(initfloorplan_end - initfloorplan_beg)/ CLOCKS_PER_SEC  <<endl;
	...
	if (flag == 1) {//如果一開始有符合
     //cout<<endl<<"init ok"<<endl;
      ...
      if(reject>5000) break;
			temperature *= 0.9999;
      finish = clock();
      if((double)(finish - start) / CLOCKS_PER_SEC > 580) break;
		}
	}
	else {//一開始沒滿足
  //cout<<endl<<"init not ok"<<endl;
 ...
 SA_end = clock();
 cout <<"SA time:"<< (double)(SA_end - SA_beg)/ CLOCKS_PER_SEC <<endl;
	/*---------------output------------------*/
  clock_t write_beg, write_end;
  write_beg = clock();
	ofstream out;
	out.open(argv[4]);
	...
	out.close();
 write_end = clock();
 cout <<"write time:"<< (double)(write_end - write_beg)/ CLOCKS_PER_SEC <<endl;
